        ...
        <div class="row">
            <div class="col-12 col-md-3">
                ...
                <ul class="nav flex-column d-none d-md-block">
                    <li class="nav-item">
                        <a class="nav-link px-0" href="{{ route('user.activity-log', $user->uuid) }}">Activity Log</a>
                    </li>
                    @can('viewAccountSettings', $user)
                        <li class="nav-item">
                            <a class="nav-link px-0" href="{{ route('user.account-settings', $user->uuid) }}">Account Settings</a>
                        </li>
                    @endcan
                    @can('viewAddressBook', [new \App\Models\UserAddressBook(), $user->uuid])
                        <li class="nav-item">
                            <a class="nav-link px-0" href="{{ route('user.address-book',  $user->uuid) }}">Address Book</a>
                        </li>
                    @endcan
                    @can('viewUserStores', [new \App\Models\Store(), $user->uuid])
                        <li class="nav-item">
                            <a class="nav-link px-0" href="{{ route('user.stores',  $user->uuid) }}">Stores</a>
                        </li>
                    @endcan
                    @if (Auth::user()->uuid === $user->uuid)
                        <li class="nav-item">
                            <a class="nav-link px-0" href="{{ route('user.notifications', $user->uuid) }}">Notifications</a>
                        </li>
                    @endif
                </ul>
